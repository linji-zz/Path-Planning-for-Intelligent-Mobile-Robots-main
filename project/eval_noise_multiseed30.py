"""eval_noise_multiseed30.py — 30x30 noise robustness: E1A (MLP+ASGS) vs E1AC (CAT+ASGS).

Same protocol as eval_noise_multiseed.py (5 seeds x 200 episodes, mean±std),
but on 30x30. The two methods differ ONLY in the network (MLP vs CAT), both use
coupled reward + ASGS, so this isolates CAT's noise-robustness contribution.
ASGS final Q-penalty (lambda=15) is applied in greedy eval for both, matching
the paper's evaluation rule. Results saved to noise_robustness_results_30x30.json.
"""
...
def main():
    ...
    mlp = load_mlp(os.path.join(PROJ, "dqn_E1_coupled_ASGS_30x30_10000.pth"))
    cat = load_cat(os.path.join(PROJ, "dqn_E1_CAT_ASGS_30x30_10000.pth"))

    results = {"noise_levels": noise_levels, "seeds": seeds,
               "episodes": episodes, "mlp_asgs": {}, "cat_asgs": {}}
    print(f"30x30 multi-seed noise eval | seeds={seeds} | episodes/seed={episodes}")
    print("-" * 60)
    for lv in noise_levels:
        ...
        results["mlp_asgs"][str(lv)] = mlp_vals
        results["cat_asgs"][str(lv)] = cat_vals
        m_mean, m_std = np.mean(mlp_vals), np.std(mlp_vals)
        c_mean, c_std = np.mean(cat_vals), np.std(cat_vals)
        print(f"noise={lv*100:>2.0f}%  E1A(MLP)={m_mean:5.1f}±{m_std:4.1f}  E1AC(CAT)={c_mean:5.1f}±{c_std:4.1f}")

    out = os.path.join(PROJ, "noise_robustness_results_30x30.json")
