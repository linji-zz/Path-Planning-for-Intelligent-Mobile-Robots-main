...
def train():
    ...
    print(f"使用设备: {device}")
    print(f"DWA环境: 15x15, 障碍物数量: {len(env.obstacles)}")
    ...
    plt.title('Reward Curve (DQN + DWA)')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig('fig1_reward_dwa.png', dpi=150)
    plt.show()
    ...
    test_env.render(path=test_path, ax=ax, title=f"DQN + DWA Path - {info.get('reason')}")
    plt.tight_layout()
    plt.savefig('path_planning_dwa.png', dpi=150)
    plt.show()

    torch.save(policy_net.state_dict(), 'dqn_dwa_10000.pth')
    print("模型已保存为 dqn_dwa_10000.pth")
