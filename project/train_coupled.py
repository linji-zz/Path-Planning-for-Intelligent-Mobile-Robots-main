"""
训练脚本 - 耦合奖励版本
直接用 env1 环境，在训练循环中替换奖励为耦合奖励
奖励 = 距离奖励 + 方向对齐奖励 + 开阔度奖励
"""
...
def train():
    ...
    for episode in range(EPISODES):
        ...
        # ===== 每个 episode 单独维护朝向角 =====
        agent_theta = 0.0  # 初始朝向角（弧度），0 表示向右
    ...
    plt.title('Reward Curve (DQN + Coupled Reward)')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig('fig1_reward_coupled.png', dpi=1000, bbox_inches='tight')
    plt.show()
    ...
    plt.title('Path Length Curve (DQN + Coupled Reward)')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig('fig2_path_length_coupled.png', dpi=1000, bbox_inches='tight')
    plt.show()
    ...
    plt.title('Distance Convergence (DQN + Coupled Reward)')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    #plt.savefig('fig3_distance_coupled.png', dpi=150)
    plt.savefig('fig3_distance_coupled.png', dpi=1000, bbox_inches='tight')
    plt.show()
    ...
    plt.title('Cumulative Success Rate (DQN + Coupled Reward)')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    #plt.savefig('fig4_success_coupled.png', dpi=150)
    plt.savefig('fig4_success_coupled.png', dpi=1000, bbox_inches='tight')
    plt.show()
    ...
    test_env.render(path=test_path, ax=ax, title=f"DQN + Coupled Reward - {info.get('reason')}")
    plt.tight_layout()
    #plt.savefig('path_planning_coupled.png', dpi=150)
    plt.savefig('path_planning_coupled.png', dpi=1000, bbox_inches='tight')
    plt.show()
    print("路径图已保存为 path_planning_coupled.png")

    torch.save(policy_net.state_dict(), 'dqn_coupled_10000.pth')
    print("模型已保存为 dqn_coupled_10000.pth")
